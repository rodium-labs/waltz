...
static void MX_ADC1_Init(void)
{
  ...
  hadc1.Instance = ADC1;
  /* PCLK2 is 84 MHz and the ADC tops out at 36 MHz, so /4 gives 21 MHz. */
  ...
  /* The internal reference needs a long sample window - 480 cycles at 21 MHz
   * is 23 us, comfortably over the 10 us minimum. */
...
static void MX_TIM2_Init(void)
{
  ...
  /* APB1 timer clock is 84 MHz; /84 -> 1 MHz tick, 1000 counts -> 1 kHz PWM. */
  htim2.Instance = TIM2;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : the five front-panel buttons */
  GPIO_InitStruct.Pin = BTN_ALL_Pins;
...
void Error_Handler(void)
{
  ...
  /* User can add his own implementation to report the HAL error state */
  __disable_irq();
  while (1) {
  }
